...
fn news_http_routes_use_sdkwork_v3_prefixes_and_resource_operation_ids() {
    assert_eq!(open_routes().len(), 16);
    assert_eq!(app_routes().len(), 37);
    assert_eq!(backend_routes().len(), 84);
    ...
    assert!(all_routes().iter().any(|route| route.method == HttpMethod::Get && route.operation_id == "trust.item.retrieve"));
    assert!(all_routes().iter().any(|route| route.method == HttpMethod::Post && route.operation_id == "factChecks.publish"));
    assert!(all_routes().iter().any(|route| route.method == HttpMethod::Post && route.operation_id == "corrections.publish"));
